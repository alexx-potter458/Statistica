...
v <- sample(587:900,10)
prob1 <- v/sum(v)
#verificare sum(prob1)
#Fatma 
a <- 13
#P(X<a)
#Lucian Bejan
proba <-sum(prob1[which(!is.na(prob1[1:(a-1)]))])
b <- 5
#P(b<X<a)
probb <- sum(prob1[(b+1):(a-1)])    #Atentie aici la NA
#P(b<X<a|X>c)
c <- 6
probc <- sum(prob1[(c+1):(a-1)])/sum(prob1[(c+1):length(prob1)])
